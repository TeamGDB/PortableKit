...
//
//   portablekit_texture_pack_tests
//   portablekit_texture_pack_tests --check <folder>
//
...
void test_location(const fs::path &root) {
    ...
    const std::string variable = portablekit::env_name("TEXTURE_PACK");
#if defined(_WIN32)
    _putenv_s(variable.c_str(), "");
#else
...
#if !defined(_WIN32)
    setenv(variable.c_str(), "1", 1);
    l = texture_pack_location(textures, kGame, "/somewhere/pack");
    check(l.source == TexturePackLocation::Source::InPlace, "<prefix>_TEXTURE_PACK=1 names no folder");
    setenv(variable.c_str(), "/elsewhere", 1);
    l = texture_pack_location(textures, kGame, "/somewhere/pack");
    check(l.source == TexturePackLocation::Source::Variable && l.folder == fs::path("/elsewhere"),
          "a folder in <prefix>_TEXTURE_PACK wins");
    unsetenv(variable.c_str());
#endif
}
