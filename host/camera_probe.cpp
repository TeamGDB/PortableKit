...
// <prefix>_FIND_CAMERA_OUT names a file the surviving list is rewritten into
...
void write_lists(const Probe &p, std::uint32_t base) {
    const char *path = portablekit::env("FIND_CAMERA_OUT");
...
// <prefix>_POKE_FLOAT=0xADDRESS:VALUE[,0xADDRESS:VALUE...] writes floats into
...
void poke_floats(psprecomp::Runtime &runtime) {
    static const char *text = portablekit::env("POKE_FLOAT");
...
std::vector<std::uint32_t> chosen_places(const std::vector<std::uint32_t> &places, const char *what) {
    static const char *which = portablekit::env("POKE_WHICH");
...
// <prefix>_FIND_FLOAT=VALUE lists every word in guest memory holding exactly that
// float, and <prefix>_POKE_FOUND=VALUE then writes a different value into all of
...
void find_and_poke_copies(psprecomp::Runtime &runtime) {
    static const char *wanted_text = portablekit::env("FIND_FLOAT");
    ...
    static const char *poke_text = portablekit::env("POKE_FOUND");
...
// <prefix>_FIND_INT32=N lists every word holding exactly that whole number, and
// <prefix>_POKE_INT32=M writes M into all of them each frame. A game that works
...
void find_and_poke_int32(psprecomp::Runtime &runtime) {
    static const char *wanted_text = portablekit::env("FIND_INT32");
    static const bool half = portablekit::env("FIND_INT16_WIDE") != nullptr;
    if (wanted_text == nullptr || *wanted_text == '\0') return;
    static const std::int32_t wanted = static_cast<std::int32_t>(std::strtol(wanted_text, nullptr, 0));
    static const char *poke_text = portablekit::env("POKE_INT32");
...
// <prefix>_FIND_STEP=N finds the camera's own yaw: the one 16-bit field whose
...
void find_step_field(psprecomp::Runtime &runtime, float turn) {
    static const char *step_text = portablekit::env("FIND_STEP");
...
void camera_frame(psprecomp::Runtime &runtime, std::uint32_t view_matrix_source) {
    ...
    static const bool enabled = portablekit::env("FIND_CAMERA") != nullptr;
    if (!enabled) return;
#if defined(PORTABLEKIT_HAS_RENDERER)
