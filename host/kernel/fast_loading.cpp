...
namespace portablekit::fast_loading {
...
bool windowed() {
    static const bool value = portablekit::env("NO_RENDER") == nullptr;
    return value;
}
...
Guards sample_guards() {
    ...
    // What only the game can tell, such as a scene that reads the disc in
    // silence while the player watches.
    if (const auto allowed = game().fast_loading_allowed) guards.game = !allowed();
#if defined(PORTABLEKIT_HAS_RENDERER)
