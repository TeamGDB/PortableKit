...
// <prefix>_TRACE_STALLS: a [stalls] line once a second, and a [slow-frame] line
// for every frame longer than <prefix>_TRACE_STALLS_MS (default 40 ms).
struct StallTrace {
    bool enabled{};
...
struct Alternate {
    std::uint32_t paths{};  // bit per NewPath named in <prefix>_PERF_ALTERNATE
...
const StallTrace &stall_trace() {
    static const StallTrace value = [] {
        StallTrace trace{};
        trace.enabled = portablekit::env("TRACE_STALLS") != nullptr;
        if (const char *text = portablekit::env("TRACE_STALLS_MS"); text != nullptr) {
