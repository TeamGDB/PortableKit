...
// <prefix>_FIND_CAMERA: finds the guest variables the game keeps its camera in.
//
// The game's camera is only observable from outside through the view matrix it
// uploads to the GE, which <prefix>_TRACE_CAMERA turns into a yaw and a turn per
...
// frames.
//
// Set <prefix>_FIND_CAMERA=1 and turn the camera; the surviving addresses are
