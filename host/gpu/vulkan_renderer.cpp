...
void VulkanRenderer::Impl::apply_texture_pack() {
    ...
    // <prefix>_TEXTURE_PACK may name the folder; otherwise the pack lives in
    // textures/<disc id>/ in the per-user data directory.
    std::filesystem::path folder = install::user_data_directory() / "textures" / portablekit::game().disc_id;
    if (const char *variable = portablekit::env("TEXTURE_PACK"); variable != nullptr) {
...
std::string VulkanRenderer::texture_pack_folder() {
    return (install::user_data_directory() / "textures" / portablekit::game().disc_id).string();
}
