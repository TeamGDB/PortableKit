...
#
# Each round: MHP3rdNative runs until it needs an overlay that has no corpus and
...
for round in $(seq 1 "$iterations"); do
    ...
    set +e
    MHP3RD_DUMP_OVERLAYS="$dump_dir" "$build_dir/bin/MHP3rdNative" > "$log_dir/run_round$round.log" 2>&1
    set -e
